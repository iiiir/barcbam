...
# camtools.py --barcodelist NA12878.cam my_barcodes.list | samtools sort - a.sort

p = argparse.ArgumentParser('camtools.py NA12878.cam AAAAAAAAAAAAAAAA | samtools view - | less')
p.add_argument('--barcodelist', '-B', action='store_true',help="query with barcode list: note it is [True|False]")
...
def clean_header(original_header):
    ...
    header_line["SQ"] = [sq_line for sq_line in original_header['SQ'] if not sq_line.get('SP',None) == 'Barcodes']
    return header_line
...
if __name__ == "__main__":
    if args.barcodelist:
        ...
        for l in open(bc):
            ...
            sam = samfile.fetch("%dbc"%(chrom+1), start, start+1, multiple_iterators=True)
            for sam_line in sam:
